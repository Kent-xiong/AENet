    ...
    def train_epoch(self, epoch):
        ...
        for i, (t1, t2, tar) in enumerate(pb):
            ...
            if epoch == 0 and i == 0:
                ...
                print(f'模型的Param总量是: {params/1e6}M')
                print(f'模型的FLOPs是: {flops/1e9}G')
    ...
    def evaluate_epoch(self, epoch):
        ...
        with torch.no_grad():
            for i, (name, t1, t2, tar) in enumerate(pb):
                ...
                out_dict = FeatureContainer()
                # if(i == 1):
                #     print("_________________________________________________________________________________________参数如下：")
                #     for name, module in self.model.named_modules():
                #         print(f"Module name: {name}")
                #         print(f"Module instance: {module}")
                #     print("______________________________________________________________________________________________:参数如上")        
                with HookHelper(self.model, fetch_dict, out_dict, hook_type='forward_out'):
                    # out = self.model(t1, t2, name)                #  #  热力图      
                    out = self.model(t1, t2)   # 为了输出热力图才将name传进去的。
