...
class DSIFNDataset(CDDataset):  
    ...
    def _read_file_paths(self):         # 这个方法实际上就是根据你下载的数据集(已经预处理裁剪好的数据集)的目录结构，来获取图片列表的。
        t1_list = sorted(glob(join(self.root, self.subset, 't1', '*.jpg')))     # glob()匹配指定路径模式的文件， 必须要sort排序，图像才能一一对应，因为你预处理好的图像可能是无序的
        t2_list = sorted(glob(join(self.root, self.subset, 't2', '*.jpg')))               
        tar_list = sorted(glob(join(self.root, self.subset, 'mask', '*.jpg')))        
